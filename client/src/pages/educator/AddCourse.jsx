...
const AddCourse = () => {
  ...
  const addLecture = async () => {
    try {
      ...
      const uploadResult = await uploadVideo(lectureDetails.lectureVideo);
        ...
        chapters.map((chapter) => {
          if (chapter.chapterId === currentChapterId) {
            const newLecture = {
              ...
              lectureUrl: uploadResult.videoPath,
                ...
                <div className="p-4">
                  {chapter.chapterContent.map((lecture, lectureIndex) => (
                    <div key={lectureIndex} className="flex justify-between items-center mb-2">
                      <span>{lectureIndex + 1} {lecture.lectureTitle} - {lecture.lectureDuration} mins - <span className="text-blue-500">Video File</span> - {lecture.isPreviewFree ? 'Free Preview' : 'Paid'}</span>
              ...
              <div className="bg-white text-gray-700 p-4 rounded relative w-full max-w-80">
                ...
                <div className="mb-2">
                  ...
                  <input
                    ...
                    className="mt-1 block w-full border rounded py-1 px-2"
                    onChange={(e) => setLectureDetails({ ...lectureDetails, lectureVideo: e.target.files[0] })}
                  />
                  {lectureDetails.lectureVideo && (
                    <p className="text-sm text-gray-600 mt-1">Selected: {lectureDetails.lectureVideo.name}</p>
                  )}
                </div>
                ...
                <button 
                  type='button' 
                  className="w-full bg-blue-400 text-white px-4 py-2 rounded disabled:opacity-50" 
                  onClick={addLecture}

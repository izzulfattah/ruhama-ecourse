...
const MyEnrollments = () => {
    ...
    // Debug helper to check progress data
    useEffect(() => {
        if (progressArray.length > 0) {
            console.log('📊 Progress Array:', progressArray.map((progress, index) => ({
                ...
                isComplete: progress.totalLectures > 0 && progress.lectureCompleted === progress.totalLectures,
                examEnabled: progress.totalLectures > 0 && progress.lectureCompleted === progress.totalLectures
            })));
        }
                        ...
                        {enrolledCourses.map((course, index) => (
                            <tr key={index} className="border-b border-gray-500/20">
                                ...
                                <td className="px-4 py-3 max-sm:text-right">
                                    ...
                                    {/* Exam button - only enabled when course is 100% complete */}
                                    <button 
                                        ...
                                        className={`ml-3 px-3 sm:px-5 py-1.5 sm:py-2 max-sm:text-xs text-white ${
                                            !progressArray[index] || 
                                            progressArray[index].totalLectures === 0 || 
                                            progressArray[index].lectureCompleted !== progressArray[index].totalLectures
